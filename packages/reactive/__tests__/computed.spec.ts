...
describe('computed', () => {
  ...
  describe('caching behavior', () => {
    it('should cache computed value and not re-compute unnecessarily', () => {
      const source = ref(1);
      const getter = jest.fn(() => source.value * 2);
    ...
    it('should re-compute only when dependencies actually change', () => {
      const source = ref(1);
      const getter = jest.fn(() => source.value * 2);
      const comp = computed(getter);
      
      comp.value; // Initial computation
      expect(getter).toHaveBeenCalledTimes(1);
      
      // Setting same value should not trigger re-computation
      source.setValue(1);
      comp.value;
      expect(getter).toHaveBeenCalledTimes(1);
      
      // Setting different value should trigger re-computation
      source.setValue(2);
      comp.value;
      expect(getter).toHaveBeenCalledTimes(2);
    });
  ...
  describe('reactivity integration', () => {
    it('should trigger watchers when computed value changes', (done) => {
      const source = ref(1);
      const comp = computed(() => source.value * 2);
  ...
  describe('performance tests', () => {
    ...
    it('should optimize updates when computed value does not change', () => {
      ...
      const getter2 = jest.fn(() => comp1.value * 2);
  ...
  describe('ReactiveFlags and internal behavior', () => {
    ...
    it('should pass oldValue parameter to getter when available', () => {
      const source = ref(1);
      const getter = jest.fn((oldValue?: number) => {
  ...
  describe('scheduler behavior', () => {
    it('should trigger dependent effects when computed changes', (done) => {
      const source = ref(1);
      const comp = computed(() => source.value * 2);
  ...
  describe('error handling and recovery', () => {
    it('should handle errors in getter gracefully and not break reactivity', () => {
      const source = ref(1);
    ...
    it('should handle errors in setter gracefully', () => {
      const source = ref(1);
      const comp = computed({
        get: () => source.value * 2,
        set: () => {
      ...
      expect(comp.value).toBe(2);
      expect(() => comp.setValue(10)).toThrow('Setter error');
      expect(comp.value).toBe(2); // Value should remain unchanged
    });
  });
